  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {groups.map((group, gi) => {
        ...
        const allComplete = group.entries.every(
          ...
          >
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                >
                  ({group.entries.length} vaccine
                  {group.entries.length !== 1 ? "s" : ""})
                </span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                {hasOverdue && (
                  <MiniTag label="Overdue" color="#DC2626" bg="#FEE2E2" />
                )}
                {allComplete && (
                  <MiniTag label="All Done" color="#16A34A" bg="#DCFCE7" />
                )}
                <span
                  style={{
                    color: "#94A3B8",
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {entry.vaccine.diseases_prevented && (
                          ...
                          >
                            Protects against:{" "}
                            {entry.vaccine.diseases_prevented.join(", ")}
                          </div>
                        )}
                        {entry.vaccination?.administered_date && (
                          ...
                          >
                            ✓ Administered:{" "}
                            {format(
                              new Date(entry.vaccination.administered_date),
                              "dd MMM yyyy",
                            )}
                            {entry.vaccination.batch_number &&
                              ` • Batch: ${entry.vaccination.batch_number}`}
                          </div>
                        )}
